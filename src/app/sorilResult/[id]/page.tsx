...
function ExamResultDetailPage() {
	...
	const answers = data.RetDataThirt; // ← "Thirt" (i нэг)
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
							<div className="p-4 bg-primary/10 rounded-xl">
								...
								<p className="text-2xl font-bold text-primary">
									{examSummary.point}/{examSummary.ttl_point}
								</p>
							</div>
										...
										{question.que_type_id === 6 ? (
											<div className="space-y-4">
												{(() => {
													...
													const columnA = questionAnswers.filter(
														(a) => a.ref_child_id && a.ref_child_id >= 1,
													);
													// Column B: Options (ref_child_id === -1)
													const columnB = questionAnswers.filter(
														(a) => a.ref_child_id === -1,
													);
													...
													return (
														<div className="space-y-3">
															...
															{columnA.map((itemA, idx) => {
																...
																const userSelectedPosition = userAnswerForA
																	? parseInt(userAnswerForA.answer)
																	: null;

																// Find which B item user selected based on position
																const userMatchedB = userSelectedPosition
																	? columnB.find(
																			(b) => b.refid === userSelectedPosition,
																		)
																	: null;

																const isCorrect =
																	userMatchedB?.answer_id ===
																	correctMatchB?.answer_id;
																		...
																		<div className="grid grid-cols-[1fr,auto,1fr] gap-4 items-center">
																			<div className="flex items-center gap-3">
																				...
																				<div className="flex-1 font-medium">
																					{safeParse(itemA.answer_name_html) ||
																						itemA.answer_name}
																				</div>
																			</div>
																					...
																					<div>
																						...
																						>
																							...
																							<div className="flex-1 font-medium text-sm">
																								{safeParse(
																									correctMatchB.answer_name_html,
																								) || correctMatchB.answer_name}
																							</div>
																							{isCorrect && (
																				...
																				{userMatchedB && !isCorrect && (
																					<div>
																						...
																						<div className="flex items-center gap-3 p-2 bg-red-50 dark:bg-red-950/30 border border-red-400 rounded-lg">
																							...
																							<div className="flex-1 font-medium text-sm">
																								{safeParse(
																									userMatchedB.answer_name_html,
																								) || userMatchedB.answer_name}
																							</div>
											...
											<div className="space-y-2">
												{questionAnswers.map((answer, idx) => {
													...
													const userOrder =
														userAnswerIndex >= 0 ? userAnswerIndex + 1 : null;
																	...
																	return (
																		<div className="p-3 bg-orange-50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-800 rounded-lg">
																			<p className="text-xs font-semibold text-orange-600 dark:text-orange-400">
																				⚠️ Та {missedCorrect.length} зөв
																				хариултыг сонгоогүй байна
																			</p>
																		</div>
																	);
																}
