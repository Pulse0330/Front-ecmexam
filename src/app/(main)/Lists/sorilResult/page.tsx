...
export default function SorilResultList() {
	const { userId } = useAuthStore();

	const { data, isLoading, isError, error } =
		useQuery<SorilresultListResponseType>({
			queryKey: ["sorilResults", userId],
				...
				<div className="text-center space-y-4 p-8 rounded-lg bg-card border border-border shadow-sm max-w-md">
					...
					<div className="space-y-2">
						...
						<p className="text-muted-foreground">
							Одоогоор дууссан сорил байхгүй байна.
						</p>
					...
					<div className="pt-4">
						<p className="text-sm text-muted-foreground">
							Та эхлээд сорил өгснөөр энд үр дүн харагдах болно.
						</p>
					</div>
				</div>
			</div>
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Нийт {exams.length} сорилын үр дүн
					</p>
				</div>
			</div>
