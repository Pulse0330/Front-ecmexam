	...
	return (
		<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
			{pastExams.map((exam, index) => {
				...
				// Create a unique key combining exam_id and index to ensure uniqueness
					...
					>
						...
						<div className="relative w-full h-56 overflow-hidden">
							{exam.filename ? (
								<>
									<Image
										src={exam.filename}
										alt={exam.soril_name || "Шалгалтын зураг"}
										fill
						...
						<CardContent className="relative z-10 space-y-3 p-5">
									...
									<div className="flex-1 min-w-0">
										...
										<p className="text-sm font-bold text-foreground truncate">
											{exam.sorildate !== "1900-01-01T00:00:00.000Z"
												? (() => {
														const date = new Date(exam.sorildate);
							...
							<div className="grid grid-cols-2 gap-3">
								...
								{/* Duration */}
								<div className="group/item relative overflow-hidden rounded-xl border-2 border-amber-100 dark:border-amber-900/30 bg-linear-to-br from-amber-50 to-amber-100/50 dark:from-amber-950/30 dark:to-amber-900/20 p-3">
									<div className="absolute top-0 right-0 w-16 h-16 bg-amber-400/10 rounded-full blur-xl" />
									<div className="relative">
										<div className="p-2 bg-linear-to-br from-amber-500 to-amber-600 rounded-lg shadow-md mb-2 inline-flex transition-transform duration-300 group-hover/item:scale-110 group-hover/item:rotate-3">
						...
						<CardFooter className="relative z-10 p-5 pt-2">
							<Button
								...
								className={`group/button w-full h-14 font-bold text-base shadow-lg rounded-xl ${
									isCompleted
										? "bg-linear-to-r from-green-600 via-emerald-600 to-teal-600 text-white"
										: "bg-linear-to-r from-orange-600 via-amber-600 to-yellow-600 text-white"
								}`}
							>
						...
						<div
							className={`absolute bottom-0 left-0 right-0 h-1.5 ${
								isCompleted
									...
									: "bg-linear-to-r from-orange-500/50 via-amber-500 to-yellow-500/50"
							}`}
						/>
