...
import { getMNExamVariants } from "@/lib/api";
import { useAuthStore } from "@/stores/useAuthStore";
import { getMNExamAttendace } from "@/lib/api";
import type {
    mnExamVariantData,
...
const VariantCard = memo(({ item, index }: VariantCardProps) => {
 ...
 const { userId } = useAuthStore();
    ...
    const variantLabel = item.variant_number ?? 0; // null → 0
...
const uniqueData = data?.RetData?.filter((item, index, self) =>
